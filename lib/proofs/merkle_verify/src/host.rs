...
mod tests {
    ...
    use loaders::loaders::{
        ...
        save_all_merkle_proofs, save_nullifiers, save_signatures,
    };
    use merkle_verifier_core::merkle_patricia::{
        verify_account_proof, verify_all_account_proofs, verify_all_storage_proofs,
    };
    use nullifier_verifier_core::nullifiers::{generate_all_nullifiers, verify_all_nullifiers};
    use signature_verifier_core::signature_verifier::{
        generate_all_signatures, verify_all_signatures,
    };
    use std::str::FromStr;
    ...
    async fn defi_inputs_validation_test() {
        ...
        let total_eth_balance: ethereum_types::U256 = verify_all_account_proofs(
        ...
        // 3. Verifying merkle proofs of all storge slots of the user and retriving their values
        ...
        let signatures_valid =
            verify_all_signatures(message, &all_signatures, &user_owned_addresses);
    ...
    #[tokio::test]
    async fn no_history_defi_inputs_validation_test() {
        let message = "Block 2";
        let all_signatures = load_signatures("signatures.json").unwrap();
        let all_nullifiers = load_nullifiers("nullifiers.json").unwrap();
        let all_merkle_proofs =
            load_all_merkle_proofs("no_history_all_merkle_proofs.json").unwrap();
        let user_owned_addresses = load_user_owned_addresses("user_owned_addresses.json").unwrap();

        let trusted_state_root = all_merkle_proofs.user_history_proof.state_root;

        // 1. Verifying all owned account proofs and getting the total eth balance of all owned accounts
        let total_eth_balance: U256 = verify_all_account_proofs(
            &all_merkle_proofs.owned_accounts_merkle_proofs,
            &user_owned_addresses,
            &trusted_state_root,
        );
    ...
    #[tokio::test]
    async fn fetch_and_validation_no_history_defi_inputs_test() {
        ...
        let user_address = user_owned_addresses[1];
        let message = "Block 2";
        ...
        let all_merkle_proofs = fetcher
            ...
            .await
            .unwrap(); // Save
        save_all_merkle_proofs(&all_merkle_proofs, "no_history_all_merkle_proofs.json").unwrap();
    ...
    #[tokio::test]
    async fn fetch_and_validation_full_defi_inputs_test() {
        //this stuff should ber provided as input to the proof
        let user_owned_private_keys: Vec<[u8; 32]> = vec![
            hex::decode("ac0974bec39a17e36ba4a6b4d238ff944bacb478cbed5efcae784d7bf4f2ff80")
                .unwrap()
