...
pub enum TrustLevel {
    Basic = 1, // data verification and score calculatoin done in TEE
    // user only forwards the TEE signed  result to the contract
    Enhanced = 2, // all data verification happend on TEE
    // user only verifys TEE signature and calcuates the score in RISC Zero
    Premium = 3, // verifcation of TLSN proofs (stateHashRoots, tradify score) on TEE,
    // user verifys TEE signature of stateHashRoots and tradify score. He then verifys , merkle proofs, nullifiers and the signature created with owned accounts
    Platinum = 4, // everything verified  and calulated by the user in RISC zero
}
